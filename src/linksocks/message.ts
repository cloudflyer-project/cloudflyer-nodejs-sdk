/**
 * LinkSocks Binary Protocol Message Handling
 * 
 * This module provides binary message serialization and deserialization
 * for the LinkSocks WebSocket protocol.
 */
...
export function packMessage(msg: BaseMessage): Uint8Array {
  ...
  switch (msg.getType()) {
    ...
    case MessageType.Connect: {
      ...
      if (connectMsg.protocol === "tcp" && connectMsg.address && connectMsg.port) {
    ...
    case MessageType.Data: {
      ...
      const dataArray = concatUint8Arrays(new Uint8Array(payload), compressedData);

      if (dataMsg.protocol === "udp") {
        const addrBytes = new TextEncoder().encode(dataMsg.address || "");
        const targetAddrBytes = new TextEncoder().encode(dataMsg.targetAddr || "");
    ...
    case MessageType.ConnectResponse: {
      ...
      const payload = [BinaryType.ConnectResponse, respMsg.success ? 1 : 0, ...channelIdBytes];
    ...
    case MessageType.Disconnect: {
      ...
      return concatUint8Arrays(header, new Uint8Array([BinaryType.Disconnect, ...channelIdBytes]));
    }
    ...
    case MessageType.ConnectorResponse: {
      ...
      const channelIdBytes = uuidToBytes(respMsg.channelId);
      const payload = [BinaryType.ConnectorResponse, ...channelIdBytes, respMsg.success ? 1 : 0];
...
export function parseMessage(data: Uint8Array): BaseMessage {
  ...
  switch (msgType) {
    case BinaryType.Auth: {
      ...
      const instance = bytesToUUID(payload.slice(1 + tokenLen + 1, 1 + tokenLen + 1 + 16));
      return {
        token,
        reverse,
        instance,
    ...
    case BinaryType.Data: {
      ...
      const dataLen =
        (payload[18] << 24) | (payload[19] << 16) | (payload[20] << 8) | payload[21];

      if (payload.length < 22 + dataLen) {
        throw new Error("Invalid data message length");
      }

      let msgData = payload.slice(22, 22 + dataLen);
      if (compression === DataCompression.Gzip) {
        msgData = ungzip(msgData) as Uint8Array<ArrayBuffer>;
      }
      ...
      if (protocol === "udp") {
        ...
        msg.address = new TextDecoder().decode(payload.slice(offset, offset + addrLen));
        offset += addrLen;
        ...
        msg.targetAddr = new TextDecoder().decode(payload.slice(offset, offset + targetAddrLen));
    ...
    case BinaryType.Connector: {
      ...
      const connectorToken = new TextDecoder().decode(payload.slice(17, 17 + tokenLen));
    ...
    case BinaryType.ConnectorResponse: {
      ...
      if (payload.length > 17) {
        ...
        if (!success) {
          msg.error = new TextDecoder().decode(payload.slice(18, 18 + tokenLen));
        } else {
          msg.connectorToken = new TextDecoder().decode(payload.slice(18, 18 + tokenLen));
        }
      }
      return msg;
    }
